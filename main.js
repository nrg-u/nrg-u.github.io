...
function submitPassword() {
    ...
    if (answer === "$" || answer === "MarkErNullKommaNullNullNullNullNull...EnCmHøy") {
...
var softClick = new Audio("/resources/soundeffects/softClick-Amplified.wav");
...
if (currentPath !== "/") {
    if (currentPath !== "/dagsrutine/"  && currentPath !== "/dagsrutine" && currentPath !== "/dagsrutine/main/" && currentPath !== "/dagsrutine/main") {
        ...
        for (var i = 0; i < cards.length; i++) {
            if (event.key == i+1) {
                if (cards[i].getAttribute("onclick")) {
                    var onclickEvent = cards[i].getAttribute("onclick").match(/'(.*?)'/)[1];
                    if (onclickEvent.charAt(0) === "/") {
                        window.location.href = onclickEvent;
                    } else {
                        eval(showPrompt(onclickEvent));
                    }
                }
            }   
        }
